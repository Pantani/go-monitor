...
nohup /bin/prometheus --config.file=/prometheus/prometheus.yml --web.listen-address=:9090 &

echo "-Set Grafana configs:"
echo "Port: " $PORT
echo "Prometheus URL: " $PROMETHEUS_URL

sed -i -e 's|${PORT}|'$PORT'|g' /etc/grafana/grafana.ini
sed -i -e 's|$PROMETHEUS_URL|'$PROMETHEUS_URL'|g' /etc/grafana/provisioning/datasources/datasource.yml
